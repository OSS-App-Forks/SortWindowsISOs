...
def parse_1_xml():
    ...
    version_string = "%s.%s" % (versions['build'], versions['spbuild'])
    ...
    if display_name is not None:
    	# DF32 Edit: Need to know if it is Eval
        ...
        if "Evaluation" in display_name.text.strip():
            display_name = display_name.text.strip().replace("Evaluation", "eval").strip()
        else:
            display_name = display_name.text.strip()
...
def archive_iso(archive_dir, iso, versions, version_string, arch_string, display_name, languages_string, verbose=False):
    ...
    # DF32 Edit: don't create 100 dirs
    # basedir = "%s/%s/%s - %s/%s/" % (archive_dir, os_type, version_string, display_name, languages_string)

    basedir = "%s/" % (archive_dir)

    # DF32 Edit: Use a more common format
    ...
    isoname = display_name.replace(' ', '_').lower()
    if "branch" in versions.keys():
        isoname = "%s_%s_%s_%s_%s.iso" % (languages_string, isoname, version_string, versions['branch'], arch_string)
    else:
        isoname = "%s_%s_%s_%s.iso" % (languages_string, isoname, version_string, arch_string)
    # DF32: PrettyEdit  Override!!
    isoname = "%s %s (build %s).iso" % (display_name, arch_string, version_string)
